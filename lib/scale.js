...
Scale.prototype.$_init = function(opt) {
  ...
  if (_config.start > _config.end) {
    this._ifError = true;
    console.error(
      '【scale】传入的「options:end」参数值不能小于「options:end」参数值'
    );
  }
  ...
  if (!_config.def) {
    this.current_def =
      Math.floor(
        ((_config.end - _config.start) / 2 + _config.start) / _config.capacity
      ) * _config.capacity;
...
Scale.prototype.$_drawScale = function() {
  ...
  let begin_num =
    this.current_def - (_config.width / 2) * (_config.capacity / _config.unit);
  let cur_x = 0;
  let cur_num = 0;
  const scale_len = Math.ceil((_config.width + 1) / _config.unit); // 刻度条数
  const real_len = Math.ceil(
    (_config.end - _config.start + 1) / _config.capacity
  ); // 实际可绘制的刻度条数
  ...
  let space_num =
    Math.ceil(begin_num / _config.capacity) * _config.capacity - begin_num;
  ...
  for (let i = 0; i < scale_len; i++) {
    ...
    if (cur_num % (_config.capacity * 10) === 0) {
      ...
      _ctx_bg.fillText(cur_num, cur_x, (_config.height * 1) / 3);
      ...
      if (real_len <= 10) {
        ...
        _ctx_bg.fillText(cur_num, cur_x, (_config.height * 1) / 2);
      }
    } else {
...
Scale.prototype.$_addEvent = function() {
  let { _canvas, _config, $_moveDraw } = this;
  let last_x = 0; //上一次x坐标
  ...
  const move = e => {
    ...
    if (ifMove) {
      ...
      this.current_def =
        this.current_def - move_x * (_config.capacity / _config.unit);
  ...
  const end = e => {
    ...
    const step = () => {
      this.current_def = slowActionfn(
        t,
        from_def,
        _config.capacity * v * 50,
        d
      );
      ...
      if (_config.openUnitChange) {
        this.current_def =
          Math.round(this.current_def / _config.capacity) * _config.capacity;
      }
      this.$_moveDraw();
      t++;
      if (t <= d) {
...
Scale.prototype.update = function(value) {
  this._point_x =
    ((value - this._config.start) / this._config.capacity) * this._config.unit; // 初始化开始位置
  this.$_moveDraw();
};
